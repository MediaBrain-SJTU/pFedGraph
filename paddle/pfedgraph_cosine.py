...
def local_train_pfedgraph(args, round, nets_this_round, cluster_models, train_local_dls, val_local_dls, test_dl, data_distributions, best_val_acc_list, best_test_acc_list, benign_client_list):
    ...
    for net_id, net in nets_this_round.items():
        ...
        if args.optimizer == 'adam':
            ...
            # 使用 Momentum 优化器以匹配 momentum=0.9
        ...
        for iteration in range(args.num_local_iterations):
            ...
            if round > 0:
                ...
                for param in net.parameters():
                    flatten_model.append(paddle.reshape(param, [-1]))
                flatten_model = paddle.concat(flatten_model)
                
                # paddle.dot, paddle.norm
                loss2 = args.lam * paddle.dot(cluster_model, flatten_model) / paddle.norm(flatten_model)
                loss2.backward()
...
for i in range(cfg['client_num']):
    ...
    # named_parameters() 用法一致
    dw.append({key : paddle.zeros_like(value) for key, value in local_models[i].named_parameters()})
    best_val_acc_list.append(0)
    best_test_acc_list.append(0)

graph_matrix = paddle.ones((len(local_models), len(local_models))) / (len(local_models)-1)
# diag 不支持直接赋值，使用 mask 或者 loop，或者 numpy 中转
# 这里用 numpy 中转比较简单
gm_np = graph_matrix.numpy()
np.fill_diagonal(gm_np, 0)
graph_matrix = paddle.to_tensor(gm_np)

for net in local_models:
    net.set_state_dict(global_parameters)
...
for net_id, net in enumerate(local_models):
    model_path = f'./models/pfedgraph_cosine_client_{net_id}_model_{time.strftime("%Y%m%d_%H%M%S")}.pth'
