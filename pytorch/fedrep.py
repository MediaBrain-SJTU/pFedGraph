...
from fedrep.config import get_args
from model import simplecnn, textcnn
from test import compute_local_test_accuracy, compute_acc
from prepare_data import get_dataloader
from attack import *

def local_train_fedavg(args, nets_this_round, train_local_dls, val_local_dls, test_dl, data_distributions, best_val_acc_list, best_test_acc_list, benign_client_list):
    ...
    for net_id, net in nets_this_round.items():
        ...
        optimizer = torch.optim.SGD(net.base.parameters(), lr=args.lr, momentum=0.9, weight_decay=args.reg)
        poptimizer = torch.optim.SGD(net.classifier.parameters(), lr = args.lr, momentum=0.9, weight_decay=args.reg)
        ...
        for iteration in range(args.num_cls_iterations):
            try:
        ...
        for iteration in range(args.num_rep_iterations):
            try:
                x, target = next(iterator)
            except StopIteration:
                iterator = iter(train_local_dl)
...
for round in range(args.comm_round):          # Federated round loop
    ...
    mean_personalized_acc = local_train_fedavg(args, nets_this_round, train_local_dls, val_local_dls, test_dl, data_distributions, best_val_acc_list, best_test_acc_list, benign_client_list)
